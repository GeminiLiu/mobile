 ...
 */
public class CreatGroupTree {
	
	static final Logger logger = (Logger) Logger.getLogger(CreatGroupTree.class);
...
//			当前用户为Demo时
			if(tuser.toLowerCase().equals("demo"))
			{
				...
				System.out.println("当前表名为："+grouptablename);
				sql.append(" order by a.C630000022");
			}
			else
			{
			...
			stm	= dataBase.GetStatement();
			System.out.println("sql语句是："+sql.toString());
    		rs	= dataBase.executeResultSet(stm,sql.toString());
			...
			while(rs.next())
			{
	 	    	...
	 	    	System.out.println(groupDnId);
	 	    	System.out.println(groupFullName);
	 	    	//生成树顶层节点
	 	    	strjs.append("tree.add(new WebFXLoadTreeItem(\""+groupName+"\",\"grouploadtree.jsp?str="+groupID+";0\",\"\",\"\",\"\",\"\",\"\",\""+groupName+":"+groupID+":"+groupFullName+":"+groupDnId+"\"));");
			}
	...
	public StringBuffer groupTree(String tuser)
	{
	 	StringBuffer sql = new StringBuffer();
	 	//实例化一个类型为接口IDataBase类型的工厂类
		IDataBase dataBase	= DataBaseFactory.createDataBaseClassFromProp();
...
//			当前用户为Demo时
			if(tuser.toLowerCase().equals("demo"))
			{
				sql.append("select C1,C630000018,C630000022,C630000021 from "+grouptablename +" where C630000025 = '0' and C630000020 = '0' order by C630000022");
			}
			else
			{
				//当前用户不为Demo时的C1
		  		...
		  		if(getUserInfoList.isSystemManagerById(tuserId))
		  		{
		  			sql.append("select a.C1,a.C630000018,a.C630000019,a.C630000037,a.C630000022,a.C630000021 from "+grouptablename+" a,"+sourcemanager+" b,"+sourceconfig+" c");
					sql.append(" where a.C630000025='0' and a.C1 = b.C650000003 and b.C650000007 = '"+tuserId+"' and not exists");
					sql.append(" (select c.C650000003 from "+sourcemanager+" c where c.C650000007 = '"+tuserId+"' and a.C630000020 = c.C650000003)");
					sql.append(" and b.C650000001 = c.source_id and c.source_name = '"+systemmanage+"'");
		  		}
		  		...
		  		{
		  			sql.append("select a.C1,a.C630000018,a.C630000019,a.C630000037,a.C630000022,a.C630000021 from "+grouptablename+" a,"+usertablename+" b");
		  			sql.append(" where a.C1 = b.C630000015 and b.C1 = '"+tuserId+"'");
		  			sql.append(" and a.C630000025='0'");
		  		}
    		...
    		if(tuser.toLowerCase().equals("demo")){  
				while(rs.next())
				{
		 	    	...
		 	    	if(String.valueOf(groupType).equals("3")){
		 	    		strjs.append("tree.add(new WebFXLoadTreeItem(\""+groupName+"\",\"grouploadtree.jsp?str="+groupID+";14\",\"\",\"\",\"\",\"\",\"\",\""+groupName+":"+groupID+":"+groupFullName+":"+groupDnId+"\"));");		 	    		
		 	    	}else{
		 	    		strjs.append("tree.add(new WebFXLoadTreeItem(\""+groupName+"\",\"grouploadtree.jsp?str="+groupID+";14\",\"\",\"\",\"\",\"\",\"\"));");
		 	    	}
				}
    		}else{
    			while(rs.next())
