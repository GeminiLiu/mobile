...
public class GroupIsSearchTreeSQL
{
	
		static final Logger logger = (Logger) Logger.getLogger(GroupIsSearchTreeSQL.class);
				...
				while(rs.next())
				{
					String sourceid 		= rs.getString("source_id");
		 	    	...
		 	    	sourcename 				= sourcename.replace(':','#');
		 	 	  	...
		 	 	  	sbf.append("tree.add(new WebFXLoadTreeItem(\""+sourcecnname+"\",\"groupsnatchsearchstree.jsp?sourceid="+sourceid+"\",\"\",\"\",\"\",\"\",\"\",\""+sourcecnname+":"+sourcename+":"+sourceid+"\"));");
				}
				sbf.append("}catch(e){}");
				sbf.append("</script>");
			...
			try
			{
				while(rs.next())
				{
					String sourceid 		= rs.getString("source_id");
		 	    	...
		 	    	sourcename 				= sourcename.replace(':','#');
		 	 	  	String sourcecnname     = rs.getString("source_cnname");
		 	 	  	sbf.append("<tree text=\""+sourcecnname+"\" src=\"groupsnatchsearchstree.jsp?sourceid="+sourceid+"\"   funpram=\""+sourcecnname+":"+sourcename+":"+sourceid+"\"/>");
				}
				rs.close();
				stm.close();
				dataBase.closeConn();
...
//			sql.append(" (select  source_id,source_parentid,source_name,source_cnname,source_orderby "); 
		...
		{
			StringBuffer sql = new StringBuffer();
//			sql.append("select a.source_id,a.source_name,a.source_cnname from "+ sourceconfig +" a,"+ sysskill +" b");
...
//					{
						sbf.append("tree.add(new WebFXLoadTreeItem(\""+groupName+"\",\"groupsnatchsearchgtree.jsp?gid="+groupID+"\",\"\",\"\",\"\",\"\",\"\",\""+groupName+":"+groupID+":"+groupParentID+"\"));");
//					}
//					else if(grouptype.equals("1"))
//					{
//						sbf.append("tree.add(new WebFXLoadTreeItem(\""+groupName+"\",\"groupsnatchsearchgtree.jsp?gid="+groupID+"\",\"\",\"\",\"\",\"\",\"\",\"\",\""+groupName+";"+groupID+"\"));");
//					}
				}
				sbf.append("}catch(e){}");
				sbf.append("</script>");
...
//					else if(grouptype.equals("0"))
//					{
						sbf.append("<tree text=\""+groupName+"\" src=\"groupsnatchsearchgtree.jsp?gid="+groupID+"\"  funpram=\""+groupName+":"+groupID+":"+groupParentID+"\"/>");
//					}
//					else if(grouptype.equals("1"))
//					{
//						sbf.append("<tree text=\""+groupName+"\" src=\"groupsnatchsearchgtree.jsp?gid="+groupID+"\"  schkbox=\""+groupName+";"+groupID+"\"/>");
//					}
				}
				rs.close();
				stm.close();
