...
public class RolesManageTempletSQL
{	 
	...
	public String getParentGroupInfo(String userid)
	{
		...
		try
		{
			while(rs.next())
			{
				...
				sbf.append("tree.add(new WebFXLoadTreeItem(\""+groupName+"\",\"rolesusergrouprelinfotree.jsp?gid="+groupID+"\",\"\",\"\",\"\",\"\",\"\",\"\",\""+groupID+";1\"));");
			}
			sbf.append("}catch(e){}");
			sbf.append("</script>");
		...
		try
		{
			while(rs.next())
			{
				String groupParentID 	= rs.getString("C630000020");
	 	    	String groupName     	= rs.getString("C630000018");
	 	 	  	String groupID       	= rs.getString("C1");
				sbf.append("<tree text=\""+groupName+"\" src=\"rolesusergrouprelinfotree.jsp?gid="+groupID+"\"  schkbox=\""+groupID+";1\"/>");
			}
			rs.close();
			stm.close();
			dataBase.closeConn();
	...
	public String getGroupAndUserInfo(String groupid)
	{
		...
		System.out.println("查询用户的SQL"+sql.toString());
		StringBuffer sbf 	= new StringBuffer();
		IDataBase dataBase	= null;
		dataBase 			= DataBaseFactory.createDataBaseClassFromProp();
		//获得数据库查询结果集
