...
import cn.com.ultrapower.eoms.user.authorization.role.hibernate.dbmanage.JDBCGetRoleCheckedUserTree;
...
public class JDBCSendScopeCheckedUserTree {

	static final Logger logger 		= (Logger) Logger.getLogger(JDBCSendScopeCheckedUserTree.class);
	...
	public String getgrouptree(String strwhere) 
	{
		...
		{
			logger.info("286 JDBCSendScopeCheckedUserTree 类中 getgrouptree(String strwhere) 方法调用GetTableProperty时出现异常！"+e.getMessage());
		}
		try {
			if (!String.valueOf(tablename).equals("")
				...
				while (rs.next()) {
					...
					if(i==0)
					{
						treeinfo.append("d.add(\"" + groupID + "\",\""
								+ groupParentID + "\",\"" + groupName
								+ "\",\"\",\"\",\"main\",\"\",\"\",\"\",'1');");
					}else
					{
						treeinfo.append("d.add(\"" + groupID + "\",\""
			...
			logger.info("287  JDBCSendScopeCheckedUserTree 类中 getgrouptree(String strwhere) 方法执行查询时出现异常！"+e.getMessage());
			return null;
		}
		finally
		{
	...
	public String getusertree(String strwhere) {
		...
		{
			logger.info("288 JDBCSendScopeCheckedUserTree 类中 getusertree(String strwhere) 方法调用GetTableProperty时出现异常！"+e.getMessage());
		}
		try {
			if (!String.valueOf(usertablename).equals("")
				...
				while (rs.next()) {
					...
					if(i==0)
					{
						treeinfo.append("d.add(\"" + userid + "\",\"" + groupid
								+ "\",\"" + userfullname
								+ "<input type='checkbox' name='User_Name' value='"
								+ userid + ";" + groupid
								+ "'>\",'','','main',\"\",\"\",\"\",'1');");
					}else
					{
			...
			logger.info("289 JDBCSendScopeCheckedUserTree 类中 getusertree(String strwhere) 方法执行查询时出现异常！" +e.getMessage());
			return null;
		}
		finally
		{
	...
	public String nodefind(String tuser)
	{
		...
		{
			logger.info("290 JDBCSendScopeCheckedUserTree 类中 nodefind(String tuser) 方法调用GetTableProperty时出现异常！"+e.getMessage());
		}
		...
		{
			logger.error("291 JDBCSendScopeCheckedUserTree 类中 nodefind(String tuser) 方法执行查询时出现异常！"+e.getMessage());
			return null;
		}
		finally
		{
	...
	public String userInfo(String user)
	{
		...
		{
			logger.info("292 JDBCSendScopeCheckedUserTree 类中 userInfo(String user)方法调用GetTableProperty时出现异常！"+e.getMessage());
		}
		try 
		{
			if (!String.valueOf(usertablename).equals("")
		...
		{
			logger.info("293 JDBCSendScopeCheckedUserTree 类中 userInfo(String user)方法执行查询时出现异常！"+e.getMessage());
			return null;
		}
		finally
		{
	...
	public String getNotInStr(String sql)
	{
		...
		{
			logger.error("294 JDBCSendScopeCheckedUserTree 类中 getNotInStr(String sql)方法执行查询时出现异常!"+e.getMessage());
			return null;
		}
		finally
		{
