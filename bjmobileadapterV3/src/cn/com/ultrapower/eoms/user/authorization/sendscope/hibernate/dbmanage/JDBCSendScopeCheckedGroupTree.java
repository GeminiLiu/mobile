...
public class JDBCSendScopeCheckedGroupTree {

	static final Logger logger 		= (Logger) Logger.getLogger(JDBCSendScopeCheckedGroupTree.class);
	/**
	 * 日期 2006-11-27
	 * 
	...
	public String getgrouptree(String strwhere) 
	{
		...
		{
			logger.info("277 JDBCSendScopeCheckedGroupTree 类中getgrouptree(String strwhere) 方法调用GetTableProperty时出现异常！");
		}
		try 
		{
			if (!String.valueOf(tablename).equals("")
					&& !String.valueOf(tablename).equals("null")) 
			{
				String sql = "select * from " + tablename
						+ " where 1=1 and C630000025='0'";
				if (!String.valueOf(strwhere).equals("")
						&& !String.valueOf(strwhere).equals("null")) 
				{
		...
		{
			logger.info("278 JDBCSendScopeCheckedGroupTree 类中getgrouptree(String strwhere) 方法执行查询时出现异常！"+e.getMessage());
			return null;
		}
		finally
		{
	...
	public String getusertree(String strwhere) 
	{
		...
		{
			logger.info("279 JDBCSendScopeCheckedGroupTree 类中getusertree(String strwhere) 方法调用GetTableProperty时出现异常！"+e.getMessage());
		}
		try
		{
					...
					&& !String.valueOf(groupusertablename).equals("null")) 
			{
				String selecttable = groupusertablename + " tgroupuser,"
						+ grouptablename + " tgroup," + usertablename
						+ " tuser";
				...
				while (rs.next()) 
				{
					...
					if(i==0)
					{
						treeinfo = treeinfo + "d.add(\"" + userid + "\",\"" + groupid
								+ "\",\"<font onclick=back_time('" + userfullname
								+ "','" + userid + "','" + groupid + "');>"
								+ userfullname + "</font>\",'','','main',\"\",\"\",\"false\",'1');";
					}else
					{
						treeinfo = treeinfo + "d.add(\"" + userid + "\",\"" + groupid
								+ "\",\"<font onclick=back_time('" + userfullname
								+ "','" + userid + "','" + groupid + "');>"
								+ userfullname + "</font>\",'','','main',\"\",\"\",\"false\",'0');";
					}
					i++;
				}
				System.out.println(treeinfo);
		...
		{
			logger.info("280 JDBCSendScopeCheckedGroupTree 类中getusertree(String strwhere) 方法执行查询时出现异常！"+e.getMessage());
			return null;
		}
		finally
		{
	...
	public String nodefind(String tuser)
	{
		...
		{
			logger.info("281 JDBCSendScopeCheckedGroupTree 类中 nodefind(String tuser) 方法调用GetTableProperty时出现异常！"+e.getMessage());
		}
		...
		try
		{
			...
			if(!sql1.toString().equals("no"))
			{
				...
				while(rs1.next())
				{
		 	    	...
		 	    	{
		 	    		treeinfo1.append("d.add(\""+groupID+"\",'0',\""+groupName+"\",\"\",\"\",\"main\",\"\",\"\",\"\",'1',\"<input type='checkbox' name='Group_Name' value='"
								+ groupID
								+ "'>"
								+ "</font>\");\n");
		 	    	...
		 	    	{
		 	    		treeinfo1.append("d.add(\""+groupID+"\",'0',\""+groupName+"\",\"\",\"\",\"main\",\"\",\"\",\"\",'0',\"<input type='checkbox' name='Group_Name' value='"
								+ groupID
								+ "'>"
								+ "</font>\");\n");
		 	    	}
			...
			while(rs.next())
			{
	 	    	...
	 	    	{
	 	    		treeinfo.append("d.add(\""+groupID+"\",\""+groupParentID+"\",\""+groupName+"\",\"\",\"\",\"main\",\"\",\"\",\"\",'1',\"<input type='checkbox' name='Group_Name' value='"
							+ groupID
							+ "'>"
							+ "</font>\");\n");
		 	    	imgflag		= "yes";
	 	    	...
	 	    	{
	 	    		treeinfo.append("d.add(\""+groupID+"\",\""+groupParentID+"\",\""+groupName+"\",\"\",\"\",\"main\",\"\",\"\",\"\",'0',\"<input type='checkbox' name='Group_Name' value='"
							+ groupID
							+ "'>"
							+ "</font>\");\n");
	 	    	}
		...
		{
			logger.error("282  JDBCSendScopeCheckedGroupTree 类中 nodefind(String tuser) 方法执行查询时出现异常！"+e.getMessage());
			return null;
		}
		finally
		{
	...
	public String userInfo(String user)
	{
		...
		{
			logger.info("283 JDBCSendScopeCheckedGroupTree 类中 userInfo(String user) 方法调用GetTableProperty时出现异常！"+e.getMessage());
		}
		try {
			if (!String.valueOf(usertablename).equals("")
					&& !String.valueOf(usertablename).equals("null")
		...
		{
			logger.info("284  JDBCSendScopeCheckedGroupTree 类中 userInfo(String user) 方法查询时出现异常！"+e.getMessage());
			return null;
		}
		finally
		{
	...
	public String getNotInStr(String sql)
	{
		...
		{
			logger.error("285 JDBCSendScopeCheckedGroupTree 类中 getNotInStr(String sql)方法执行查询时出现异常！"+e.getMessage());
			return null;
		}
		finally
		{
