...
public class JDBCGetRoleCheckedUserTree 
{
	
	static final Logger logger 		= (Logger) Logger.getLogger(JDBCGetRoleCheckedUserTree.class);
	...
	{
		JDBCGetRoleCheckedUserTree groupUserTree = new JDBCGetRoleCheckedUserTree();
		System.out.println(groupUserTree.userInfo("Demo"));
	}
	...
	public String getgrouptree(String strwhere) 
	{
		...
		{
			logger.info("244 JDBCGetRoleCheckedUserTree 类中 getgrouptree(String strwhere) 方法中调用GetTableProperty时出现异常！");
		}
		...
		try 
		{
					...
					&& !String.valueOf(tablename).equals("null")) 
			{
				...
				{
					sql = sql + " and " + strwhere + " order by C630000022";
				}
				...
				while (rs.next()) 
				{
					...
					if(i==0)
					{
						treeinfo = treeinfo + "d.add(\"" + groupID + "\",\""
		...
		{
			logger.info("245 JDBCGetRoleCheckedUserTree 类中 getgrouptree(String strwhere) 方法中执行查询时出现异常！");
			return null;
		}
		finally
		{
	...
	public String getusertree(String strwhere) 
	{
		...
		{
			logger.info("246 JDBCGetRoleCheckedUserTree 类中 getusertree(String strwhere) 方法中调用GetTableProperty时出现异常！");
		}
		try 
		{
		...
		{
			logger.info("247 JDBCGetRoleCheckedUserTree 类中 getusertree(String strwhere) 方法中查询时出现异常！");
			return null;
		}
		finally
		{
	...
	public String nodefind(String tuser)
	{
		...
		{
			logger.info("248 JDBCGetRoleCheckedUserTree 类中 nodefind(String tuser) 方法中调用GetTableProperty时出现异常！");
		}
		...
		{
			logger.error("249 JDBCGetRoleCheckedUserTree 类中 nodefind(String tuser) 方法中执行查询时出现异常！");
			return null;
		}
		finally
		{
	...
	public String userInfo(String user)
	{
		...
		{
			logger.info("250 JDBCGetRoleCheckedUserTree 类中 userInfo(String user) 方法中调用GetTableProperty时出现异常！");
		}
		try 
		{
			if (!String.valueOf(usertablename).equals("")
...
//				System.out.println(treeinfo);
		...
		{
			logger.info("251 JDBCGetRoleCheckedUserTree 类中 userInfo(String user) 方法中执行查询时出现异常！");
			return null;
		}
		finally
		{
	...
	public String getNotInStr(String sql)
	{
		...
		{
			logger.error("252  JDBCGetRoleCheckedUserTree 类中 getNotInStr(String sql) 方法中执行查询时出现异常！"+e.getMessage());
			return null;
		}
		finally
		{
