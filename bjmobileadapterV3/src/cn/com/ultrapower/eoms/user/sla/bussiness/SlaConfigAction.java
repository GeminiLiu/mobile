...
public class SlaConfigAction extends HttpServlet {
	...
	{		
		...
		if(groupgrand.isDuplicate(slaconfigpo)){
			res.sendRedirect("../roles/slaconfigadd.jsp?error=001&id="+id);
		}else{
			groupgrand.insertSlaConfig(slaconfigpo);		
			res.sendRedirect("../roles/slaconfigadd.jsp?targetflag=yes");
		}		
	}
}
