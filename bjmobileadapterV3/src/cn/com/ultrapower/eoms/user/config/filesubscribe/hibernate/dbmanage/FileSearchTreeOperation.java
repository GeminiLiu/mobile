 ...
 * @author xuquanxing
 */
public class FileSearchTreeOperation
{
	//	从配置文件中取表名
	...
	{
		System.out.println("xuquanxing2");
		...
		try
		{
			while(res.next())
			{
				try
				{
					...
					resultbuff.append("tree.add(new WebFXLoadTreeItem(\"" + sourcname+ "\",\"filesearchnextxml.jsp?gid=" + sourceid + "\",\"\",\"\",\"\",\"\",\"\",\""+sourcname+":"+sourceid+"\"));");
	...
	{
		System.out.println("xuquanxing1=");
		StringBuffer resultbuff = new StringBuffer();
		...
		System.out.println("xuquanxing2="+sql);
		IDataBase dataBase	= DataBaseFactory.createDataBaseClassFromProp();
		Statement stm		= null;
		ResultSet res		= null;
		stm	= dataBase.GetStatement();
		...
		try
		{
			while(res.next())
			{
				try
				{
					...
					resultbuff.append("<tree text=\"" + sourcname + "\" src=\"filesearchnextxml.jsp?gid=" + sourceid + "\" funpram=\"" + sourcname + ":" + sourceid + "\"/>");
	...
	public String getFileSourceRootTree(String userid)
	{
		...
		System.out.println(sql.toString()+"查询资源树sql");
//		if(type.equals("1"))
//		{
...
//		sql.append(" order by source_id,source_parentid");
	...
	public String getSonFileSourceRootTree(String userid,Long id)
	{
		StringBuffer resultbuff = new StringBuffer();
		StringBuffer sql 		= new StringBuffer();
		sql.append("select distinct a.source_id,a.source_parentid,a.source_cnname from "+ sourceconfig +" a,"+ roleskill +" b");
		sql.append(" where a.source_id = b.C610000008 and b.c610000007='"+userid+"'");
		sql.append(" and a.source_parentid = '"+id+"'");
		sql.append(" and a.source_type like '%7;%'");
		sql.append(" union ");
		sql.append("select distinct a.source_id,a.source_parentid,a.source_cnname from "+ sourceconfig +" a,");
		sql.append(""+RemedyTrolesusergrouprel+" RemedyTrolesusergrouprel,"+RemedyTrolesskillmanage+" RemedyTrolesskillmanage,"+RemedyTgroupuser+" RemedyTgroupuser,");
		sql.append( rolemanagetable + " rolemanagetable");
		sql.append(" where a.source_id = RemedyTrolesskillmanage.C660000007");
		sql.append(" and a.source_parentid = '"+id+"'");
		sql.append(" and a.source_type like '%7;%'");
		sql.append(" and RemedyTrolesskillmanage.C660000006 = RemedyTrolesusergrouprel.c660000028");
		sql.append(" and ((RemedyTrolesusergrouprel.C660000027=RemedyTgroupuser.C620000027 and RemedyTrolesusergrouprel.C660000026 is null) or (RemedyTrolesusergrouprel.C660000027=RemedyTgroupuser.C620000027 and RemedyTrolesusergrouprel.C660000026='"+userid+"')) and RemedyTgroupuser.C620000028='"+userid+"'");
		sql.append(" and rolemanagetable.c1=RemedyTrolesusergrouprel.c660000028");
		IDataBase dataBase	= DataBaseFactory.createDataBaseClassFromProp();
		Statement stm		= null;
		ResultSet res		= null;
