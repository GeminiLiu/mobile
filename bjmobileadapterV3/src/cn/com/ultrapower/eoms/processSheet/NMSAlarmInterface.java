...
public class NMSAlarmInterface {
	...
	static{
		...
		processMap = intcfg.getProcessMap(Contents.NMSPROCESS);
	}
	
	public String getAllBMCTable()
	{
		return this.getPrefix() + Contents.BMCSPLIT + this.getBmcTable();
	}
	/**
	 * 
	 * @return 获取工单前缀字符
	...
	public Map<String,String> getMethodMap(String method)
	{
		...
		map = intcfg.getProcessMap(Contents.NMSPROCESS,method);
		return map;
	}
	...
	public List<Map<String, String>> getBasefieldInfo(String methodName,String type)
	{
		...
		List<Map<String, String>> list = intcfg.getBaseFieldInfo(Contents.NMSPROCESS, methodName);
		for(int i=0;i<list.size();i++)
		{
			currentMap = list.get(i);
			if(currentMap.get(Contents.TYPE)!=null && currentMap.get(Contents.TYPE).toString().equals(type))
			{
