 ...
 * @author BigMouse
 */
public class DealProcess extends Process
{
	/**
	 * 获取处理环节的Process集合
	 * 
	...
	public List getProcessInfoList(String sqlString) throws Exception
	{
		...
		Map fieldMap = fmConfig.getFieldMap("Deal");

		// 读取所有的字段名，替换sql语句中的*，拼成最后的sql语句
		List fieldList = fmConfig.getFieldList("Deal");
		...
		while (rs.next())
		{
			DealProcessInfo dpInfo = new DealProcessInfo();
			...
			for (int i = 0; i < fieldsName.length; i++)
			{
				...
				switch (fModel.getFieldType())
				{
					case FieldType.INT:
					{
						// 找到并调用set方法，向字段赋值
						Class[] c = { int.class };
						Method method = DealProcessInfo.class.getMethod(modelName, c);
