 ...
 * @author BigMouse
 */
public class DealLink extends Link
{
	/**
	 * 获取处理环节的Link集合
	 * 
	...
	public List getLinkListInfo(String sqlString) throws Exception
	{
		...
		Map fieldMap = fmConfig.getFieldMap("DLink");
		
		// 读取所有的字段名，替换sql语句中的*，拼成最后的sql语句
		List fieldList = fmConfig.getFieldList("DLink");
		...
		while (rs.next())
		{
			DealLinkInfo dlInfo = new DealLinkInfo();
			...
			for (int i = 0; i < fieldsName.length; i++)
			{
				...
				switch (fModel.getFieldType())
				{
					case FieldType.INT:
					{
						...
						Class[] c = { int.class };
						Method method = DealLinkInfo.class.getMethod(modelName, c);
