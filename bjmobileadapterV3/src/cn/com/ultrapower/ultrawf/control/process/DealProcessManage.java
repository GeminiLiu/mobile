...
public class DealProcessManage {
	public List<DealProcessModel> getList(RDParameter p_rDParameter)
	{
		p_rDParameter.addIndirectPar("is_notnull","1",2);
      	RowSet 	m_rowSet			= OpDB.getDataSetFromXML("DealProcess.BaseOn_Condition",p_rDParameter) ;
      	Row 	m_Rs				= null;
      	List<DealProcessModel>	m_List	= new ArrayList<DealProcessModel>();
      	for (int i = 0;i<m_rowSet.length();i++) 
		{
      		m_Rs = m_rowSet.get(i);
      		DealProcessModel m_Model 	= (DealProcessModel) setModelValue(m_Rs);
      		m_List.add(m_Model);
		}    
		return m_List;
	}

	public DealProcessModel getOneModel(RDParameter p_rDParameter)
	{
		p_rDParameter.addIndirectPar("is_notnull","1",2);
      	RowSet 	m_rowSet			= OpDB.getDataSetFromXML("DealProcess.BaseOn_Condition",p_rDParameter) ;
      	DealProcessModel m_Model	= null;
      	if (m_rowSet.length()>0)
      	{
      		Row 	m_Rs			= null;
	  		m_Rs = m_rowSet.get(0);
	  		m_Model 	= (DealProcessModel) setModelValue(m_Rs);
      	}
		return m_Model;
	}
	
	public DealProcessModel getOneModel(String p_ProcessID,String p_ProcessType)
	{
	...
	{
		DealProcessModel m_Model 	= new DealProcessModel(); 
		...
		m_Model.setFlagGroupSnatch(p_Rs.getString("FlagGroupSnatch"));
