...
public class SourceDirectoryTypeInterface {

static final Logger logger 			= (Logger) Logger.getLogger(SourceDirectoryTypeInterface.class);
	...
	public Document getMenuTree(String username, SourceConfigInfoBean sourceConfigBean,List beanList,String showType)
	{
		...
		try
		{
			...
			if(getGroupSourceRoleList == null)
			{
				System.out.println("您输入的条件在数据库中无法查询出数据！");
			}
		}
		catch(Exception e)
		{
			System.out.println("关联查询组信息，资源信息，技能信息出现异常！");
			getGroupSourceRoleList = null;
		}
		if(getGroupSourceRoleList!=null&&showType.endsWith("0"))
		{
...
//		sourceconfigBean.setSource_type("4;");
		...
		list1.add(sourceatt);
		GetRole getRole = new GetRole();
		SourceDirectoryTypeInterface sourceRoleGrandInterface = new SourceDirectoryTypeInterface();
		...
		try 
		{
			...
			if(doc!=null)
			{
				XMLWriter writer = new XMLWriter(new FileOutputStream(new File("E:/testgrouptype.xml")));
				writer.write(doc);
				writer.close();
			}
		} catch (Exception e1) 
