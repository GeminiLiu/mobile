...
public class UserDepTreeImpl extends TreeManager{
	 ...
	 * @return
	 */
	public String getDepUserListXml(String whereValue,String isSelectType){
		List<DtreeBean> dtreeBeanList = getDepOrUserList(whereValue,isSelectType);
	...
	public String getDepUserXmlOftargetDataArr(String parentid,String isSelectType,String targetDataArr){
		if(parentid=="")
			return "";
		//List<DtreeBean> menuDtreeList = getDepUserData(parentid,isSelectType);
		List<DtreeBean> menuDtreeList = getDepUserDataOftargetDataArr(parentid,isSelectType,targetDataArr);
		return this.createDhtmlxtreeXml(menuDtreeList, parentid);
	}
	 ...
	 * @return
	 */
	private List<DtreeBean> getDepUserData(String parentid,String isSelectType){
		if(parentid=="")
			return null;
	...
	private List<DtreeBean> getCustomDepUserData(String parentid,String isSelectType,String formCustSendTree){
		...
		List<DtreeBean> dtreeList = new ArrayList<DtreeBean>();
		StringBuffer sql = new StringBuffer();
		sql.append(" select userdepinfo.pid, userdepinfo.orgname text, userdepinfo.orgtype ");
		sql.append(" from bs_t_sm_customorganize custorg,");
		sql.append(" (select pid, depname orgname, '1' orgtype");
		sql.append(" from bs_t_sm_dep t");
		sql.append(" where status = 1 and parentid = '"+parentid+"' ");
		sql.append(" union all");
		sql.append(" select sysuser.pid pid, sysuser.fullname orgname, '2' orgtype");
		sql.append(" from bs_t_sm_userdep userdep,bs_t_sm_user sysuser, bs_t_sm_dep dep");
	 ...
	 * @return
	 */
	private List<DtreeBean> getDepUserDataOftargetDataArr(String parentid,String isSelectType,String targetDataArr){
		if(parentid=="")
			return null;
		List<DtreeBean> dtreeList = new ArrayList<DtreeBean>();
		StringBuffer sql = new StringBuffer();
	...
	public List<DtreeBean> getDepOrUserListOfcust(String whereValue,String formCustSendTree){
		...
		sql.append(" (select sysuser.pid, sysuser.fullname text, '2' orgtype from bs_t_sm_user sysuser where sysuser.fullname like '%"+whereValue+"%'" );
		sql.append(" union all");
	 ...
	 * @return
	 */
	public List<DtreeBean> getDepOrUserList(String whereValue,String isSelectType){
		if(whereValue=="")
			return null;
		List<DtreeBean> dtreeList = new ArrayList<DtreeBean>();
		StringBuffer sql = new StringBuffer();
		if(!isSelectType.equals("0")){//如果查询部门,则不显示人
			...
			sql.append(" where sysuser.fullname like '%"+whereValue+"%'");
			sql.append(" union all ");
		}
