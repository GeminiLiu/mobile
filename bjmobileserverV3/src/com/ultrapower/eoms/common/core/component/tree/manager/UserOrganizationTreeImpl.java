 ...
 * 摘要:
 */
public class UserOrganizationTreeImpl implements DynamicDataService {
	...
	public List<DtreeBean> getChildList(String parentid) {
		...
		List<DutyOrganization> orgList = organizationService.getOrgListByUserId(userSession,"1");
		List<DtreeBean> dtreeList = new ArrayList<DtreeBean>();
	   
		if(orgList!=null && orgList.size()>0){
			for(DutyOrganization org : orgList){
				DtreeBean dtreeBean = new DtreeBean();
				dtreeBean.setId(DutyConstants.DUTY_FORM_CLASS_NAME+","+org.getPid());
