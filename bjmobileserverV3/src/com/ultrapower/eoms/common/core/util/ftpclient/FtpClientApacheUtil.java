...
public class FtpClientApacheUtil {
	...
	/**
	 * @param usernamestr FTP登录账号
	 * @param passwordstr FTP登录密码
	 * @param hoststr FTP服务器主机地址
	 * @param portstr FTP服务器端口
	 * @param remoteFileDirstr FTP服务器上的相对路径,下载地址
	 * @param remoteBakFileDirstr FTP服务器上的相对路径,上传地址
	 */
	public FtpClientApacheUtil(String usernamestr,String passwordstr,String hoststr,int portstr,String remoteFileDirstr,String remoteBakFileDirstr){
	...
	*/ 
	public static boolean uploadFile(String filename,InputStream input) {
		...
		try {
			...
			ftpClient.changeWorkingDirectory(remoteBakFileDir);
	...
	public static boolean downFile(String localPath) {
		...
		try {
			...
			for(int i=0;i<fs.length;i++){
				...
				System.out.println("下载到本地的文件："+localPath + "/" + fs[i].getName());
				OutputStream is = new FileOutputStream(localFile);
				boolean down = ftpClient.retrieveFile(fs[i].getName(), is);
				is.close();
				if(down==true)
